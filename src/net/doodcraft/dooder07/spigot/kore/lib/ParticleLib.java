...
public class ParticleLib {
    ...
    public enum ParticleType {
        ...
        EXPLOSION_NORMAL("explode", 0, 17), EXPLOSION_LARGE("largeexplode", 1, 1), EXPLOSION_HUGE("hugeexplosion", 2, 0), FIREWORKS_SPARK("fireworksSpark", 3, 2), WATER_BUBBLE("bubble", 4, 3), WATER_SPLASH("splash", 5, 21), WATER_WAKE("wake", 6, -1), SUSPENDED("suspended", 7, 4), SUSPENDED_DEPTH("depthsuspend", 8, 5), CRIT("crit", 9, 7), CRIT_MAGIC("magicCrit", 10, 8), SMOKE_NORMAL("smoke", 11, -1), SMOKE_LARGE("largesmoke", 12, 22), SPELL("spell", 13, 11), SPELL_INSTANT("instantSpell", 14, 12), SPELL_MOB("mobSpell", 15, 9), SPELL_MOB_AMBIENT("mobSpellAmbient", 16, 10), SPELL_WITCH("witchMagic", 17, 13), DRIP_WATER("dripWater", 18, 27), DRIP_LAVA("dripLava", 19, 28), VILLAGER_ANGRY("angryVillager", 20, 31), VILLAGER_HAPPY("happyVillager", 21, 32), TOWN_AURA("townaura", 22, 6), NOTE("note", 23, 24), PORTAL("portal", 24, 15), ENCHANTMENT_TABLE("enchantmenttable", 25, 16), FLAME("flame", 26, 18), LAVA("lava", 27, 19), FOOTSTEP("footstep", 28, 20), CLOUD("cloud", 29, 23), REDSTONE(
                "reddust", 30, 24), SNOWBALL("snowballpoof", 31, 25), SNOW_SHOVEL("snowshovel", 32, 28), SLIME("slime", 33, 29), HEART("heart", 34, 30), BARRIER("barrier", 35, -1), ITEM_CRACK("iconcrack_", 36, 33), BLOCK_CRACK("tilecrack_", 37, 34), BLOCK_DUST("blockdust_", 38, -1), WATER_DROP("droplet", 39, -1), ITEM_TAKE("take", 40, -1), MOB_APPEARANCE("mobappearance", 41, -1);
    ...
    static {
        ...
        try {
            DoodLogger.log("Kore", "Server major/minor version: " + v);
            if (v < 1.7) {
                DoodLogger.log("Kore", "Hooking into pre-Netty NMS classes");
                netty = false;
                ...
                DoodLogger.log("Kore", "Hooking into Netty NMS classes");
                packetClass = getNmsClass("PacketPlayOutWorldParticles");
                if (v < 1.8) {
                    DoodLogger.log("Kore", "Version is < 1.8 - using old packet constructor");
                    ...
                    DoodLogger.log("Kore", "Version is >= 1.8 - using new packet constructor");
            ...
            DoodLogger.printError("Kore", "&cFailed to initialize NMS components!", ex);
            compatible = false;
        }
    }
    ...
    private static Object getHandle(Entity entity) {
        try {
            ...
            DoodLogger.printError("Kore", "&cFailed to send packet!", ex);
            return null;
        }
    }
    ...
    private static Class<?> getNmsClass(String name) {
        ...
        try {
            ...
            DoodLogger.printError("Kore", "&cFailed to load NMS class " + name + "!", ex);
        }
        return clazz;
    }
     ...
     * @param count  the number of particles to worlds
     * @param radius the radius of the particles
     */
    public ParticleLib(ParticleType type, double speed, int count, double radius) {
        this.type = type;
        this.speed = speed;
        this.count = count;
        this.radius = radius;
     ...
     *
     * @param location the location to worlds the particle effect at
    ...
    private Object createPacket(Location location) {
        try {
            ...
            DoodLogger.printError("Kore", "&cFailed to construct particle effect packet!", ex);
    ...
    public void sendToLocation(Location location) {
        try {
            ...
            DoodLogger.printError("Kore", "&cFailed to send packet!", e);
        }
    }
